 ...
 * @param {*} hookError - The error from the failed hook (may be non-Error value)
 * @returns {Error} The error object for the skipped test
 */
function createHookSkipError(hookTitle, hookError) {
  // Handle non-Error thrown values (e.g., throw null, throw undefined, throw "string")
  var hookErrorMessage;
  var hookErrorStack;

  if (hookError instanceof Error) {
    hookErrorMessage = hookError.message;
    hookErrorStack = hookError.stack;
  } else if (hookError === null || hookError === undefined) {
    hookErrorMessage = String(hookError);
    hookErrorStack = undefined;
  } else {
    hookErrorMessage = String(hookError);
    hookErrorStack = undefined;
  }
  ...
  var errorMessage =
    'Test skipped due to failure in "' + hookTitle + '": ' + hookErrorMessage;
  var testError = new Error(errorMessage);
  if (hookErrorStack) {
    testError.stack = hookErrorStack;
  }
  return testError;
}
...
Runner.prototype.failAffectedTests = function (suite, hookError, hookTitle) {
  ...
  var suitesToProcess = [suite];

  while (suitesToProcess.length > 0) {
    var currentSuite = suitesToProcess.pop();

    // Fail all unexecuted tests in this suite
    ...
    // Add child suites to the stack for processing
    for (var i = currentSuite.suites.length - 1; i >= 0; i--) {
      suitesToProcess.push(currentSuite.suites[i]);
    }
  }
};
